...
function useMinimizer({
	...
	let latex = "$" + String.raw({ raw: latexFormat(mdno, n, format) }) + " $";

	if (mdno.length === 1 && mdno[0][1] === 0) {
		latex = "$ 1 $";
	}

	// validation
	const isMintermStrValid =
		isValidMintermsString(mintermsStr) &&
		minterms.every((minterm) => minterm < 1 << n);
...
function App() {
	...
	const { minterms, n, mdno, latex, isMintermStrValid, isNValid } =
		useMinimizer({
			mintermsStr,
			format,
			numberOfVars,
...
function useVeitch(numOfVars: number) {
	...
	let numOfRows = 1 << Math.floor(numOfVars / 2);
	let numOfCols = 1 << Math.floor(numOfVars / 2);
...
function VeitchDiagram({
	...
	const grid = Array(numOfRows).fill(Array(numOfCols).fill(Math.random()));

	// create ranges of variables x1 to xn
	// seperately for odd and even numbers
	const segments = [];
	for (let i = 1; i <= numberOfVars; i++) {
		segments.push([]);
		for (let j = 0; j < 1; j++) {
			// segments[i - 1].push([]);
		}
	...
	const getMinterm = (i: number, j: number) => {
		...
		>
			{grid.map((row, i) =>
				row.map((_: any, j: number) => (
					<div className="border grid place-items-center" key={i + j}>
						{"mt"}
					</div>
				))
			)}
